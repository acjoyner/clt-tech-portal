...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="p-20 font-black uppercase text-center">Loading...</div>;
  ...
  return (
    <div className="p-4 md:p-8 bg-white min-h-screen text-black">
      ...
      <section className="mb-16">
        <h2 className="text-3xl font-black mb-4 uppercase italic text-yellow-600 underline leading-none">New Quote Leads</h2>
        <div className="border-4 border-black overflow-x-auto shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          <table className="w-full text-left min-w-[700px]">
            ...
            <tbody>
              {quotes.filter(q => q.status?.toLowerCase() === 'pending').map(quote => (
                <tr key={quote.id} className="border-b-2 border-black">
                  <td className="p-4 font-bold">{quote.customer_name}<br/><span className="text-[10px] text-gray-400 font-normal">{quote.email}</span></td>
      ...
      {/* ACTIVE WORKBENCH */}
      <section className="mb-16">
        <h2 className="text-3xl font-black mb-4 uppercase italic text-blue-700 underline leading-none">Active Workbench</h2>
        <div className="border-4 border-black overflow-x-auto shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          <table className="w-full text-left min-w-[700px]">
            ...
            <tbody>
              {quotes.filter(q => q.status?.toLowerCase() === 'active').map(quote => (
                <tr key={quote.id} className="border-b-2 border-black">
                  ...
                  <td className="p-4 text-center">
                    <button onClick={() => archiveRepair(quote.id)} className="bg-green-500 border-2 border-black px-2 py-1 font-black text-[10px] uppercase text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-green-600">Archive</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="mb-16">
        <div className="flex justify-between items-end mb-4 text-black">
          ...
          <div className="flex items-center gap-4">
            <button onClick={exportProfits} className="bg-blue-600 text-white border-4 border-black px-4 py-2 font-black uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">Download CSV</button>
            <div className="bg-green-400 border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              <span className="text-xs font-black uppercase block leading-none mb-1">Net Profit</span>
          ...
          <div className="bg-white border-8 border-black p-8 max-w-md w-full shadow-[16px_16px_0px_0px_rgba(0,0,0,1)]">
            {activeModal.type === "notes" && (
              <>
                ...
                <p className="text-xs font-bold text-blue-600 mb-2 uppercase">Customer: {activeModal.data.customer_name}</p>
                <div className="mb-4 text-[10px] font-mono bg-gray-100 p-2 border-2 border-black">
                  Current Notes: {activeModal.data.notes || "None recorded"}
                </div>
                <textarea id="m_notes" defaultValue={activeModal.data.notes} className="w-full border-4 border-black p-4 font-bold min-h-[150px] mb-6 focus:bg-yellow-50 outline-none text-black" placeholder="Add update notes here..." />
                <div className="flex gap-4">
                  <button onClick={() => saveNotes(activeModal.data.id, (document.getElementById("m_notes") as HTMLInputElement).value)} className="flex-1 bg-black text-white p-4 font-black uppercase border-4 border-black">Save Update</button>
                  <button onClick={() => setActiveModal(null)} className="flex-1 border-4 border-black font-black uppercase text-black">Close</button>
                </div>
              </>
            )}
            {/* OTHER MODALS (Specs/Publish) remain as before */}
          </div>
        </div>
      )}
    </div>
