...
interface InventoryItem {
  created_at: string; // Changed to string as it comes from Supabase as ISO string
  id: number;
  brand: string;
  model: string;
...
export default function AdminDashboard() {
  ...
  const totalRevenue = soldHistory.reduce((acc, item) => acc + (item.sale_price_listing || 0), 0);
  const totalCost = soldHistory.reduce((acc, item) => acc + (item.purchase_price || 0), 0);
  ...
  const uploadImage = async (e: React.ChangeEvent<HTMLInputElement>, itemId: number) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const fileName = `${itemId}-${new Date().getTime()}.${file.name.split(".").pop()}`;
    const { error } = await supabase.storage.from("laptop-photos").upload(fileName, file);
    if (!error) {
      const { data: { publicUrl } } = supabase.storage.from("laptop-photos").getPublicUrl(fileName);
      await supabase.from("inventory").update({ image_url: publicUrl }).eq("id", itemId);
      setRefreshSignal((s) => s + 1);
    }
  };
  ...
  return (
    <div className="p-8 bg-white min-h-screen text-black">
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%" minHeight={300}>
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ backgroundColor: "#fff", border: "4px solid black", color: "#000" }} />
      ...
      <section className="mb-16">
        ...
        <div className="border-4 border-black overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          <table className="w-full text-left">
            ...
            <tbody>
              {quotes.filter((q) => q.status === "purchased").map((quote) => (
      ...
      <section className="mb-16">
        ...
        <div className="border-4 border-black overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          <table className="w-full text-left">
            ...
            <tbody>
              {inventory.filter((i) => i.status !== "sold").map((item) => (
                <tr key={item.id} className="border-b-2 border-black">
                  ...
                  <td className="p-4 text-center">
                    <label className="cursor-pointer bg-black text-white px-3 py-1 text-[10px] font-black border-2 border-black uppercase hover:bg-white hover:text-black">
                      ...
                      <input type="file" className="hidden" onChange={(e) => uploadImage(e, item.id)} />
                    </label>
                  </td>
                  <td className="p-4 text-center space-x-2">
                    <button onClick={() => setActiveModal({ type: "specs", data: item })} className="bg-yellow-400 border-2 border-black px-2 py-1 font-black text-[10px] uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-black">Specs</button>
      ...
      {/* PROFIT HISTORY (THE GRAVY) */}
      <section className="mb-16">
        <div className="flex justify-between items-end mb-4">
          <h2 className="text-3xl font-black uppercase italic text-black">Profit History</h2>
      ...
      {/* INCOMING LEADS */}
      <section className="mb-16">
        <h2 className="text-3xl font-black mb-4 uppercase italic text-blue-700 underline">Incoming Leads</h2>
        <div className="border-4 border-black overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          <table className="w-full text-left">
            <thead className="bg-gray-100 border-b-4 border-black font-black uppercase text-sm">
              <tr>
                <th className="p-4">Customer</th>
                <th className="p-4">Laptop Details</th>
                <th className="p-4 text-center">Action</th>
              </tr>
            </thead>
            <tbody>
              {quotes.filter((q) => q.status === "new").map((quote) => (
                <tr key={quote.id} className="border-b-2 border-black">
                  <td className="p-4 font-bold">{quote.customer_name}</td>
                  <td className="p-4">{quote.laptop_details}</td>
                  <td className="p-4 text-center space-x-2">
                    <button onClick={() => setActiveModal({ type: "buy", data: quote })} className="bg-green-500 border-2 border-black font-black px-4 py-1 text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">BUY</button>
                    <button onClick={() => handleBuyLead(quote, 0)} className="bg-blue-500 border-2 border-black font-black px-4 py-1 text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">RECYCLE</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activeModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50">
          <div className="bg-white border-8 border-black p-8 max-w-md w-full">
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <input id="m_bat" defaultValue={activeModal.data.battery_health} className="w-full border-2 border-black p-2 font-black text-green-700" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-gray-400">SSD %</label>
                    <input id="m_ssd" defaultValue={activeModal.data.ssd_health} className="w-full border-2 border-black p-2 font-black text-blue-700" />
                  </div>
                </div>
                <button onClick={() => saveSpecs(activeModal.data.id, {
                  cpu: (document.getElementById("m_cpu") as HTMLInputElement).value,
                  battery_health: (document.getElementById("m_bat") as HTMLInputElement).value,
                  ssd_health: (document.getElementById("m_ssd") as HTMLInputElement).value,
                })} className="w-full bg-yellow-400 border-4 border-black p-4 font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">Save & Update</button>
